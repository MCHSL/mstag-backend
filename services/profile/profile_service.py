...
def create_profile(user_id, username):
	...
	cursor.execute("INSERT INTO player_profile(id, username) VALUES(%s, %s)",
	               (user_id, username))
	sql_conn.commit()
...
def get_user_by_name(username):
	...
	cursor.execute("SELECT * FROM player_profile WHERE username=%s",
	               (username, ))
	a = cursor.fetchone()
	if a:
		return a
...
def get_friend_invites_for_user(id):
	...
	cursor.execute(
	    "SELECT inviter_id FROM friend_invite WHERE invitee_id = %s", (id, ))
...
def add_friend_to_user(user_id, friend_id):
	...
	cursor.execute("INSERT INTO friend VALUES (%s, %s) ON CONFLICT DO NOTHING",
	               (user_id, friend_id))
	cursor.execute("INSERT INTO friend VALUES (%s, %s) ON CONFLICT DO NOTHING",
	               (friend_id, user_id))
	sql_conn.commit()
...
def remove_friend_from_user(user_id, friend_id):
	...
	cursor.execute(
	    "DELETE FROM friend WHERE player_id = %s AND friend_id = %s",
	    (user_id, friend_id))
	cursor.execute(
	    "DELETE FROM friend WHERE player_id = %s AND friend_id = %s",
	    (friend_id, user_id))
...
def add_invite_to_user(invitee_id, inviter_id):
	...
	cursor.execute(
	    "INSERT INTO friend_invite VALUES (%s, %s) ON CONFLICT DO NOTHING",
...
def accept_friend_invite(user_id, inviter_id):
	...
	cursor.execute(
	    "SELECT 1 FROM friend_invite WHERE inviter_id = %s AND invitee_id = %s",
	    (inviter_id, user_id))
	if cursor.fetchone():
		cursor.execute(
		    "DELETE FROM friend_invite WHERE inviter_id = %s AND invitee_id = %s",
		    (inviter_id, user_id))
...
def update_last_seen(user_id):
	...
	cursor.execute(
	    "UPDATE player_profile SET last_seen = %s WHERE id = %s", (int(
...
rrh = ServiceRequestHandler("profiles",
                            "profile_queue",
                            on_request,
                            broadcasts=["presence"])
rrh.run()
